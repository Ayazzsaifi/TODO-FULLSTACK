async function getTodos() {
    ...
    const response = await axios.get("http://localhost:3000/todos", { headers: { token } })
...
addTodo.addEventListener("click", async function () {
    if (todoInput.value === "") {
        ...
        const response = await axios.post("http://localhost:3000/todo", { title: newTodo }, { headers: { token } })
        todoInput.value = ""
        getTodos()
    }
})